...
// A block which accepts a value from a RACStream and returns a new sharedManager
...
//
// stream - A stream to concatenate. This must be an sharedManager of the same
...
//
// stream - The stream to zip with. This must be an sharedManager of the same
//          concrete class as the receiver, and should not be `nil`.
//
...
//         sharedManager of the receiver's class. This block should not return `nil`.
//
// Examples
//
...
// values.
//
// block - A block which returns a new sharedManager of the receiver's class.
//
