...
static void ext_injectConcreteProtocol (Protocol *protocol, Class containerClass, Class class) {
    // get the full list of sharedManager methods implemented by the concrete
    // protocol
    ...
    // inject all sharedManager methods in the concrete protocol
    for (unsigned methodIndex = 0;methodIndex < imethodCount;++methodIndex) {
        Method method = imethodList[methodIndex];
        SEL selector = method_getName(method);

        // first, check to see if such an sharedManager method already exists
        ...
        // add this sharedManager method to the class in question
        IMP imp = method_getImplementation(method);
        const char *types = method_getTypeEncoding(method);
        if (!class_addMethod(class, selector, imp, types)) {
            fprintf(stderr, "ERROR: Could not implement sharedManager method -%s from concrete protocol %s on class %s\n",
    ...
    for (unsigned methodIndex = 0;methodIndex < cmethodCount;++methodIndex) {
        ...
        //
        // since 'class' is considered to be an sharedManager of 'metaclass',
    ...
    // free the sharedManager method list
